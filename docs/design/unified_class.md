...
| Shape ID | XrJson 已用 `xr_gc_get_shape_id(&gc_header)` 把 shape ID 嵌进 GC header `extra` 字段做 IC |

**关键洞察**：要做的不是"造新基础设施"，而是把 Tier A 类型**迁移到已有的 Tier B 基础设施上**。
...
- 保留：`XR_TCLASS`、`XR_TFUNCTION`、`XR_TCFUNCTION`、`XR_TMODULE`、`XR_TSHAPE`、`XR_TBLOB`、`XR_TCELL`、`XR_TBOUND_METHOD`（内部基础设施）
...
- **不**做 V8 风格 hidden class transition（用户类一旦 `finalize` 后 shape 不可变；动态加字段仍只对 Json 开放）
...
└─ XR_TAG_PTR + heap_type=XR_T<infra>    (XR_TCLASS / XR_TFUNCTION / XR_TCFUNCTION /
                                          XR_TMODULE / XR_TBLOB / XR_TCELL / XR_TSHAPE /
...
| **Json** | XR_TJSON | 大 body（动态字段表 + shape ID） | P3 | 已用 shape，最复杂 |
| **Array** | XR_TARRAY | 大 body（typed data buffer） | P3 | 性能最敏感 |
| **Map** | XR_TMAP | 大 body（hash table） | P3 | 同上 |
| **Set** | XR_TSET | 大 body（hash set） | P3 | 同上 |
| **保留**: Coroutine、CoroPool、Closure、CFunction、Module、Class、Shape、Cell、BoundMethod、Blob、StringRaw | （内部）| —— | 不迁移 | 不对用户暴露 |
...
### R4 [MID] — Shape ID 与 XrClass 的关系

XrJson 使用 `XrShape` + `xr_gc_get_shape_id()`（存在 GC header `extra` 字段）做 IC。XrInstance 不使用 XrShape，直接 `inst->klass` 拿到 XrClass，从 `XrClass.field_symbol_to_index` 查字段。两者并存且语义重叠。

**对策**：
- **方案 A**（保守）：保持现状，XrJson 继续用 XrShape（动态字段），XrInstance 用 XrClass（静态字段）。两者 IC 条目用 `(klass *or* shape_id, offset)` 区分。
- **方案 B**（激进）：把 XrShape 与 XrClass 合并——XrClass 内嵌一个 default shape，动态加字段时 `klass->shape = transition(...)`。这对 Json 也变成"普通类"，可被继承。

**推荐**：方案 A 起步（最小化改动），方案 B 作为单独 follow-up（任务 084）。
...
### Phase 0 — 设计冻结与基础设施（1 天）

**目的**：在动任何 native 类型之前，把 unified 路径的"骨架"搭好。
...
**新 opcode**：`OP_NEW`（暂与现有 OP_NEW 共存或扩展现有 OP_NEW）。
...
**验收**：所有现有测试（ctest 116/116 + regression 307/307）通过；新增 1-2 个测试验证带 native_body 的 class 能正常分配 / traverse / destroy。
...
**改动**：
- 删除 `XrException` C struct（保留 `XR_TEXCEPTION` 一段时间作为别名）
...
- `XR_TEXCEPTION` GC tag 保留为别名（指向 Exception class），下一阶段彻底删除
...
**目标类型**：DateTime、Range、Logger、Iterator、EnumValue、EnumType、Regex、NetConn、NetListener、Task、BigInt、Channel、StringBuilder、Tuple、Bytes。
...
5. 删除 `XR_T<NAME>` GC tag（或保留为别名一段时间）
...
**目的**：迁移 Array、Map、Set、Json 这四个性能极敏感的类型。

**关键挑战**：`arr[i]`、`m["k"]`、`json.field` 等热路径不能退化。
...
**改动**：
- 删除所有 `XR_T<NAME>` GC tag 别名
- 删除所有 `xr_*_register_native_type` 函数（改名为 `xr_*_register_class` 或合并到一个统一注册）
...
- 删除 `@native` 关键字的前端解析（或保留为 deprecated warning）
...
| Phase | 时间 | 输出 | 解锁 |
|-------|------|------|------|
| Phase 0 | 1 天 | 基础设施 + OP_NEW 扩展 | —— |
| Phase 1 | 1 天 | Exception 迁移 | **081 Phase 1b** |
| Phase 2 | 2-3 天 | 15 个 cold native 类型 | 081 Phase 2（部分 enum 依赖） |
| Phase 3 | 1 周 | Array / Map / Set / Json | —— |
| Phase 4 | 1-2 天 | 清理 + 文档 | —— |
| **总计** | **~2 周** | | |
...
1. **Q1**：`XR_TEXCEPTION` 等过渡别名保留多久？建议 Phase 1-3 期间保留，Phase 4 删除。
2. **Q2**：Phase 3 是否真的迁移 Array/Map/Set/Json，还是把它们作为"compiler-internal special class"保留？建议**全部迁移**——只要保留专用 opcode 做字面量优化，性能不受影响。
3. **Q3**：`@native` 关键字是否在前端语法中保留？建议**完全删除**（用户原则：不保留旧接口）。
4. **Q4**：AOT 扩展 tag (`XR_TAG_ARRAY` 等) 是否保留？建议在 Phase 3 期间删除（与 hot collection 迁移同步）。
5. **Q5**：方案 A vs 方案 B（XrShape 与 XrClass 合并）是否在 Phase 3 之后立即做？建议作为 084 单独任务，**不**放在本任务范围。
6. **Q6**：Tuple 是否依然作为字面量优化（`(1, 2, 3)` 不走 OP_NEW）？建议保留——性能敏感。
...
- **关键参考代码**（按重要性）：
  ...
  3. `src/runtime/object/xnative_type.{c,h}` — 现有 register_native_type
  4. `src/runtime/object/xexception.{c,h}` — 081 Phase 1a 留下的 Exception 实现（迁移起点）
  5. `src/vm/xvm_dispatch_object.inc.c` — OP_GETPROP / OP_SETPROP fast path
  6. `src/vm/xvm_dispatch_invoke.inc.c` — OP_INVOKE 与 IC 集成
  7. `src/vm/xic_field.{c,h}` + `src/vm/xic_method.{c,h}` — IC 实现
  8. `src/runtime/gc/xcoro_gc_traverse.c` + `src/runtime/gc/xgc.c` — GC 集成
  9. `src/aot/xrt_class.{h}` + `src/aot/xrt_value.h` — AOT runtime（部分已 unified）
