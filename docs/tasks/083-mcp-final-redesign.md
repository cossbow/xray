...
- ~~缺少真实 stdio 协议端到端测试~~ 已完成主要路径（transport、lifecycle、parse error、unknown method、notification、runner stdout isolation、resources/templates/list、resources/read、prompts/list、prompts/get、mixed NDJSON、tools/call invalid params / structuredContent）。
- 新增 method 或 tool 时仍需同步补 transcript smoke test。
