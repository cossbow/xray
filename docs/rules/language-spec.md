...
> 若需输出 stderr，使用 `io.stderr.write(...)`（`io` 模块）。若需不换行输出，使用 `io.stdout.write(...)`。
...
| 函数 | 签名 | 说明 |
|---|---|---|
| `typeof(x)` | `(any) -> string` | 返回类型名字符串，如 `"int"`、`"Color"`、`"Tuple2"` |
| `x is T` | 表达式 | 运行时类型检查（编译期可做类型窄化，详见 §3.8） |
...
- 基本类型（小写）：`"int"`、`"float"`、`"string"`、`"bool"`、`"null"`、`"function"`
- 容器类型（大写）：`"Array"`、`"Map"`、`"Set"`、`"Json"`、`"Channel"`、`"Bytes"`
- 元组：`"Tuple2"`、`"Tuple3"` 等（按 arity）
- 枚举值：返回枚举名（如 `"Color"`、`"Direction"`）
- 类实例：返回类名（如 `"Point"`、`"Animal"`）

> 统一使用 `typeof` 返回字符串表示的类型名。

### 13.4 协程

不提供全局协程辅助函数：

- **sleep**：使用 `time.sleep(ms)`（需 `import time`）
- **启动协程**：使用 `go` 表达式（见 §10.2），返回 `Task<T>`
...
> 取整 / 平方根类方法只在 `float` 上提供；int 通过 `i.toFloat()` 升格后再调用。
...
| `toString()` | `() -> string` —— UTF-8 解码为字符串（与 `String.toBytes()` 互逆）；无效 UTF-8 透传原始字节 |
| `iterator()` / `entries()` | for-in 协议 |

> Hex / Base64 / 多字节整数读写：通过 stdlib 模块（如 `base64`、`encoding`）完成，**不**作为 Bytes 实例方法存在。
>
> 字符串 ↔ 字节互转：`s.toBytes()` 和 `b.toString()` 是规范入口，互为逆运算（前提是字节是合法 UTF-8）。
...
`a..b` 是**半开区间** `[a, b)`：`start` 包含、`end` 不包含。所有方法/属性遵循同一语义，与 `for-in` 迭代一致。

| 成员 | 类型 / 说明 |
|--|--|
| `start` | `int`（属性），起点（包含） |
| `end` | `int`（属性），终点（**不**包含） |
| `length` | `int`（属性），元素数，等于 `end - start`（step=1）或 ⌈(end-start)/step⌉ |
| `includes(x)` | `(int) -> bool`，`start <= x < end` 且 `(x - start) % step == 0` |
| `toArray()` | 物化为 `Array<int>` |
| `toString()` | `"start..end"` 或 `"start..end:step"` |

```xray
let r = 1..10               // [1, 10) — 9 个元素
r.start                     // 1
r.end                       // 10
r.length                    // 9
r.includes(1)               // true (start 包含)
r.includes(9)               // true (最后被迭代的值)
r.includes(10)              // false (end 不包含)
for (i in 1..10) { ... }    // 迭代 9 次：1, 2, ..., 9
```

### 14.12 `DateTime` 方法

通过 `import datetime` 获得；构造走模块工厂 `datetime.create(...)` / `datetime.now()` / `datetime.fromTimestamp(ms)`。

| 成员 | 说明 |
|--|--|
| `year` / `month` / `day` | 日期分量（**属性**，无括号） |
| `hour` / `minute` / `second` / `millisecond` | 时间分量（属性） |
| `weekday` / `yearday` | 周内序号 / 年内序号（属性） |
| `timestamp` | Unix 毫秒时间戳（属性） |
| `format(fmt)` | 按格式串渲染为字符串 |
| `toISOString()` | ISO-8601 字符串 |
| `toString()` | 默认字符串形式 |
| `add(amount, unit)` / `diff(other, unit)` | 加减时长 / 时间差 |
| `toUTC()` / `toLocal()` | 时区转换 |
